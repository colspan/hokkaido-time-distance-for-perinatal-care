...
export default class GeoJsonLoader{
  ...
  getLoader(){
    ...
    // 複数ファイルを非同期読み込み
    var promises = []
    this.options.geodataFiles.forEach(d => {
      var p = new Promise((resolve, reject) => {
        ...
        d3.json(d, (error, loaded) => {
          ...
          // TopoJSONデータ展開
          var geodataFieldname = Object.keys(loaded.objects)[0]
          var geojson = topojson.feature(loaded, loaded.objects[geodataFieldname])
          var exceptionCommunes = self.options.exceptions; // 対象外の市町村
          var removeList = []
          var communes = []
          ...
          resolve({geojson:geojson,communes:communes,idMap:idMap})
        })
      })
      promises.push(p)
    })
