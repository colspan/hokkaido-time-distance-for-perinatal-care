...
export default class GeoStatisticalData {
    constructor(inputData, geojsonObj) {
        ...
        this.dataArray = []
        inputData.forEach(x => {
            var communeName = x[this.csvKeys[0]] // 1列目は自治体名(制約)
            if (communeName == null || communeName.length < 2) return // 文字列が短過ぎたらスキップ
            var communeIds = this.idMap[communeName] // 自治体IDを取得
    ...
    getByColumnName(columnName){
        ...
        var format = function(x) {
            if (isNaN(x)) return x
            var formatStr = (+x % 1 === 0 && +x % 1 === 0 ? ',.0f' : '0.4f')
        ...
        var max = d3.max(dataArray, getValue)
        var min = d3.min(dataArray, getValue)
        var domain, range
        if (min < 0 && max < 0) {
            domain = [min, max]
            range = ["white", "#ff5722"]
        } else if (min < 0 && max >= 0) {
            if (getValue(dataArray[1]) > 0 && getValue(dataArray[0]) / getValue(dataArray[1]) > 3.0) {
                // 1位と2位の比率が3倍を超えるとき
